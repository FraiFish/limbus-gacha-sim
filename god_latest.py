...
class MainSim: 
    ...
    # To-do list
    # To-Do 1(top priority): figure out GUI with PyQt6
    ...
    # draw once
    # weighted pool (favored banners)
    # weird and very primal GUI using customtkinter(will change this like right now)
...
app.title("Limbus gacha")
label = ctk.CTkLabel(app, text="Limbus", font=('Arial', 18))
label.pack(pady=50)

class newClass():
    switch = 0
    resultpull = ctk.CTkLabel(app)


def button_function():
    if newClass.switch > 0:
        newClass.resultpull.destroy()
    MainSim.draw1(1)
    newClass.resultpull = ctk.CTkLabel(app, text=f"your pull: {MainSim.results_str}")
    newClass.resultpull.place(x=120, y=200)
    MainSim.results.clear()
    MainSim.results_str.clear()
    newClass.switch += 1
...
button = ctk.CTkButton(master=app, 
                       text="Extract 1\n130 🌹"
, command=button_function, font=('Arial', 18))
