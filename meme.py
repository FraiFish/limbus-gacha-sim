...
class MainSim: 
    ...
    # actual words instead of just outputting pull results 
    ...
    def pullone(): # Standard extraction
        MainSim.results.append(random.choices([MainSim.EGO[random.randint(0, 8)].name, MainSim.OOO[random.randint(0, 14)].name, MainSim.OO[random.randint(0, 19)].name, MainSim.O[random.randint(0, 11)].name], [1.2996, 2.8995, 12.8, 83.0004], k=1))
    #footnote: k is the amount of variables it wants to output
    def tenth(): # 10th pull 
        MainSim.results.append(random.choices([MainSim.EGO[random.randint(0, 8)].name, MainSim.OOO[random.randint(0, 14)].name, MainSim.OO[random.randint(0, 19)].name], [1.2996, 2.8995, 95.8009], k=1))
    ...
    def test2(): # testing taking info from objects in a class
        ee = 0
        while ee < 10:
            MainSim.results.append(MainSim.OOO[0].name) # this is exactly how i want to do the thing
            ee += 1
        print(MainSim.results)
